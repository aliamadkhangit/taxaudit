...
export default function TrustSection() {
  ...
  return (
    <motion.section
      ...
      className="py-20 bg-linear-to-b from-gray-50 to-white relative overflow-hidden"
    >
      ...
      <div className="absolute inset-0 overflow-hidden pointer-events-none">
        <motion.div
          ...
          className="absolute -top-40 -right-40 w-80 h-80 bg-indigo-100/30 rounded-full blur-3xl"
        ></motion.div>
        <motion.div
          animate={{ y: [0, 10, 0], x: [0, -8, 0] }}
          transition={{ duration: 11, repeat: Infinity, ease: "easeInOut" }}
          className="absolute -bottom-40 -left-40 w-80 h-80 bg-indigo-100/20 rounded-full blur-3xl"
        ></motion.div>
      </div>
      ...
      <div className="max-w-7xl mx-auto px-5 sm:px-8 lg:px-12 relative z-10">
        ...
        >
          <motion.span
            variants={fadeItem}
            className="text-indigo-600 font-semibold text-sm uppercase tracking-wider bg-indigo-50 px-4 py-1.5 rounded-full inline-block"
          >
            Why Choose Us
          </motion.span>
          <motion.h2
            variants={fadeItem}
            className="text-3xl md:text-4xl lg:text-5xl font-bold text-gray-900 mt-4 leading-tight"
          >
            Trusted by professionals across Pakistan
          </motion.h2>
          <motion.div
            variants={fadeItem}
            className="w-20 h-1 bg-indigo-600 mx-auto mt-6 rounded-full"
          ></motion.div>
          <motion.p
            variants={fadeItem}
            className="text-gray-500 mt-5 text-lg max-w-2xl mx-auto"
          >
        ...
        >
          {trustPoints.map((point, idx) => (
            ...
            >
              ...
              <div
                className={`absolute top-0 left-0 w-full h-1 bg-linear-to-r ${point.color} rounded-t-2xl transform scale-x-0 group-hover:scale-x-100 transition-transform duration-300 origin-left`}
              ></div>

              <div className="flex items-start gap-4">
                {/* Icon container with gradient background */}
                <div
                  className={`shrink-0 w-12 h-12 rounded-xl bg-linear-to-br ${point.color} flex items-center justify-center shadow-md group-hover:shadow-lg transition-all`}
                >
        ...
        <motion.div
          variants={fadeItem}
          className="mt-16 bg-indigo-900 rounded-2xl p-8 text-center text-white shadow-xl"
        >
